...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        double xcoord = Math.abs(vx-ux); 
        double ycoord = Math.abs(vy-uy); 
        
        //Using Pythagorean Formula, calculate distance 
   ...
   */
  //Euclidean distance between all cities that have a direct link
  //Sets weight for corresponding edges in the graph - correct distances will then be displayed in GUI
  public void computeAllEuclideanDistances() {
      //u is starting point, v is ending point
      for (Vertex v: getVertices()) 
          
          //List of adjacent edges = adjacency list 
          //Finds all adjacent edges of vertex v 
          for (Edge e: v.adjacentEdges)
              e.distance = computeEuclideanDistance(e.source.x, e.source.y, e.target.x,
                                                          e.target.y); 
  }
   ...
   */
  //Implement the algorithm starting at city s 
  //Update distance and prev instance variables of vertex objects
  //Do not use priority queue to store vertices that need to be revisited 
  //Keep vertices on a list and scan through it to find minimum 
  //The method is inspired by the pseudocode for Dijkstra's algorithm 
  //in figures 9.31 and 9.32, pages 379 and 381 of the Weiss textbook 
  public void doDijkstra(String s) {
            
      //LinkedList will store vertices 
      LinkedList<Vertex> vertices = new LinkedList<>(); 
      
      //Initialize the distance to infinity and known to false 
      //To figure out how to implement infinity in Java: 
      //https://stackoverflow.com/questions/12952024/how-to-implement-infinity-in-java
      for (Vertex v: this.getVertices()){
          double infinity = Double.POSITIVE_INFINITY; 
           v.distance = infinity; //v.dist
           v.known = false;  //v.known = false;
           v.prev = null;
           vertices.add(v); 
      }
             
       //Initialize starting vertex 
      //Method getVertex passes in a string, use this       
      Vertex start = getVertex(s); //s.distance = 0; 
      start.distance = 0; 
      start.known = true; 
      
      //While there is still an unknown vertex 
      while (!vertices.isEmpty()){
          ...
          for (int i = 0; i <vertices.size(); i++){                            
               if (minDistance > vertices.get(i).distance){
                   minDistance = vertices.get(i).distance; 
                   min = vertices.get(i); 
               }               
           }
           
           //Declares vertex known 
           vertices.remove(min); 
           start = min;  
           start.known = true; 
           ...
           for (Edge e: start.adjacentEdges) 
              if (!e.target.known && 
                  (e.distance + start.distance < e.target.distance)){
                  e.target.prev = start; 
                  
                  //Adds distances successively 
                  e.target.distance = start.distance + e.distance;  
   ...
   */
  //Uses distance and prev instance variables of vertex objects 
  //to find shortest path between s and t
  //Result: list of Edge objects, needs to work for repeated runs 
  //Re-initialize booking fields in vertices stored in vertex nodes 
  //Based on Figure 9.30 on page 378 Weiss Textbook 
  public List<Edge> getDijkstraPath(String s, String t) {
    doDijkstra(s);
    
    //Stores edges to print out final path 
    LinkedList<Edge> totPath = new LinkedList<Edge>(); 
    Vertex v = getVertex(t);  
    
    while (v.prev != null){
        v.distance = computeEuclideanDistance(v.prev.x, v.prev.y, v.x, v.y);       
        Edge addEdge = new Edge (v.prev, v, v.distance); 
        totPath.addFirst(addEdge); 
